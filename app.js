...
// --- state ---
var state = {
    page: 'landing',       // 'landing' | 'program' | 'area' | 'parameter' | 'category' | 'document'
    program: null,
    area: null,
    param: null,
    category: null,
    fileIndex: null,
    currentPath: []        // for folder hierarchy
};
...
function getQueryParams() {
    ...
    return {
        ...
        // currentPath is not stored in URL; we keep it in memory
    };
}
...
function navigateTo(page, data) {
    ...
    // if page is 'landing', we redirect to index.html
    if (page === 'landing') {
        window.location.href = 'index.html';
        return;
    }
    // For other pages, update query string and push state
...
window.addEventListener('popstate', function(event) {
    if (event.state && event.state.state) {
        ...
        state.page = 'program'; // always program page (since nasa program-specific file)
...
// --- render functions (almost same as original, but adapted) ---
...
function updatePageTitle() {
    ...
    if (state.page === 'area' && state.area) {
        ...
        title = programName + ' — ' + (PROGRAMS[programName] || '') + ' | OMSC';
    }
...
function render() {
    ...
    var node;
    // if program is not set, default to landing (but should not happen in program pages)
    if (!state.program) {
    ...
    } else if (state.fileIndex === null || state.fileIndex === undefined) {
        state.page = 'category';
    } else {
        state.page = 'document';
    }
...
// when DOM ready, load data and init
